...
- Responses use an `OCResult<T>` envelope, confirmed 22 Sep: success `{ "code": 0, "msg": "success", "data", "timestamp", "success": true }`; API errors come with **HTTP 200** and `{ "code": 40001, "msg", "data": null, "success": false }`. The gateway's own auth errors (HTTP 401) have `msg` and numeric `code` but no `success` field. The client checks `success` if present, else `code == 0`.
- `POST /api/v1/dex/market/price` body is a JSON array `[{ "binanceChainId": "56", "tokenContractAddress": "0x..." }]` (undocumented; found by trial, `fixtures/web3/market-price-*.json`).
- **`referencePrice` is `tokenPrice / tokenToShareRatio`**, not a market quote (unverified item 1, answered). Use Finnhub as the reference; treat `referencePrice` as the per-share on-chain price.
- Aggregator quotes need `userWalletAddress` for Ondo **and xStocks** (error 40001 otherwise); the Tape and probe use `TAPE_QUOTE_WALLET` (the agent wallet's public address, not a secret). xStocks on BSC return 40374 (no liquidity) for $100 and have prices days old.
...
- **Equity tokens trade via RFQ** (EIP-712 signature plus settlement polling), not the normal swap path, per the docs. But every bStocks and Ondo aggregator quote (22 and 23 Sep) comes back `vendorName: LiquidMesh`, `executionMode: "SWAP"`, and the filling venue in `dexRouterList` changes between quotes for the same token (RFQ makers such as `Rfq Halfmoon` or AMM-style `Metric`). The registry's `path` is the issuer's documented path, not how an order fills; which path a funded swap takes is unverified until T9's first trade.
